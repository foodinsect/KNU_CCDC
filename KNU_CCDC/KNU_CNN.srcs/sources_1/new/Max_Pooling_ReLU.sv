...
module Max_Pooling_ReLU(
    ...
    output reg [11:0] data_out,               // MaxPooling + ReLU result
    output reg ready_o                        // Ready signal indicating completion
    );
    ...
    always @(posedge clk_i or negedge rstn_i) begin
        if (!rstn_i) begin
            ...
            relu_en <= 1'b0; 
            ready_o <= 1'b1;                  // Indicate that the result is ready
        end else begin
            ready_o <= 1'b0;                  // Clear the ready signal when not valid
        end
    end
